...
class PlayList {
    ...
    public boolean add(Track track) {
        ...
        if (this.size == this.maxSize) { // checks if have free space.
            return false;
        } else {
            this.tracks[size] = track; // if there a space add track and make new space.
    ...
    public void removeLast() {
        ...
        if (this.size > 0) { // checks if there any tracks.
            this.tracks[size - 1] = null; // remove track from last place.
            this.size = this.size - 1; // reduce size by 1.
        }
    }
    ...
    public int indexOf(String title) {
        //// replace the following statement with your code
        for (int i = 0; i < this.size; i++) {
            if (this.getTrack(i).getTitle() == title) {
                return i;
            }
        }
        return -1;
    ...
    public boolean add(int i, Track track) {
        //// replace the following statement with your code
        if (i <= this.size && i >= 0 && this.size <= this.maxSize) {
            if (i == this.size) {
                this.add(track);
            } else {
    ...
    public void add(PlayList other) {
        //// replace this comment with your code
        if (this.getSize() + other.getSize() <= other.maxSize) {
